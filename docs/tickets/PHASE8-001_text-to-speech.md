...
さらに踏み込むなら、`FrameSequencer` を**抽象基底クラス `SequencerBase`** として設計し、`AutoSinger` / `PhonemePlayer` / `VowelPresetPlayer`（Phase 2 の母音プリセット遷移）を全て派生クラスとして実装する 3 層統合が望ましい。`SequencerBase` が「`Frame[]` のキューイング」「AudioContext クロックでの fire ループ」「stop / pause / seek」「`onFrameChange` コールバック」を提供し、派生クラスは `generateFrames(input): Frame[]` の 1 メソッドだけを実装する。具体的には `AutoSinger.generateFrames(melodyConfig)` / `PhonemePlayer.generateFrames(text)` / `VowelPresetPlayer.generateFrames(vowelId)` という対称的な API になり、Phase 2 のボタン UI から Phase 8 のテキスト読み上げまでが同じ抽象に乗る。`TransitionManager`（Worklet 側のサンプル精度補間器）はこの 3 層の下に位置する 4 層目で、`SequencerBase.fireFrame()` から無条件に呼ばれる単一の出口になる。現在の設計では「メインスレッド側 sequencer 群」「Worklet 側 transition manager」「両者を繋ぐ postMessage」が層として分離していないため、Phase 4 と Phase 8 で同じパイプラインを 2 度書き直す羽目になる。
...
加えて、`scheduleTransition` を Phase 1 から導入していれば Phase 4 Auto Sing の実装構造が根本から変わっていた。現在の Auto Sing は `requestAnimationFrame` ベースの rAF ループ（`phrase-manager.ts` 内）でメインスレッド側からビブラート LFO・ポルタメント補間・F0 ramp を 16 ms 間隔で生成し、毎フレーム postMessage で送る方式である。これは「タブ非アクティブ時に rAF が throttle される」「16 ms 粒度のジッタが LFO に乗る」「メインスレッド GC の影響を直接受ける」という 3 つの欠点を抱えている。`scheduleTransition` が Phase 1 から存在し、かつ Frame 配列に「区間内のサンプル精度補間カーブ」（linear / exponential / sine LFO）を含められる設計だったなら、Auto Sing は**フレーズ開始時に全 Frame をまとめて Worklet に送るバッチ方式**になり、メインスレッドの rAF ループは完全に廃止できる。Worklet 側は受け取った Frame 配列をサンプル単位で消化するだけになり、ビブラートもポルタメントも Worklet 内のタイマで生成される。これにより `phrase-manager.ts` / `expression-engine.ts` の rAF 駆動コードは消滅し、メインスレッド側は「Frame 生成」「再生開始指示」「停止指示」だけを担う薄い層になる。

韻律パラメータの統一管理も、Phase 1 で `Prosody` 名前空間を作っておくべきだった。現在は F0 がスライダー直接、Auto Sing は phrase-manager の中、Phase 8 では phoneme-timeline の中、と 3 箇所に分散している。Phase 1 で `Prosody.compute(frames, options)` という関数を 1 つだけ用意し、F0 / 強度 / 持続時間の計算を全フェーズで再利用する設計にすれば、Phase 8 の F0 ルールは Phase 4 と共通化できた。具体的には `Prosody` モジュールが (a) 基準 F0 曲線（ピッチアクセント / メロディノート列）(b) ビブラート LFO（深さ / 周波数 / 立ち上がりカーブ）(c) ポルタメント / グライド補間 (d) 強度エンベロープ (e) 持続時間スケーリング、の 5 種を「Frame[] への変換器」として提供し、入力に応じて任意組合せで適用する。Auto Sing のビブラートもテキスト読み上げの F0 下降パターンも、同じ `Prosody.applyVibrato(frames, opts)` / `Prosody.applyPitchContour(frames, opts)` の合成として表現でき、結果として Auto Sing の `expression-engine.ts` は不要になる。Phase 8 の `computeF0` ルールも `Prosody.applyJapaneseAccent(frames, accentType)` という関数 1 つに集約される。
...
**境界の明確な定義**としては、`src/text/` は「言語表記 → 音素列（持続時間や F0 を持たない素の `PhonemeId[]`）」までを担当し、それ以降の「音素列 → 韻律付き Frame 配列」変換は `src/prosody/` に置く。これにより `text-parser.ts` は完全に決定的で副作用ゼロのテーブル参照モジュールになり、`prosody/timeline-builder.ts`（現 `phoneme-timeline.ts` 相当）が「音素列 + アクセント型 + 速度 + 基準 F0 → Frame[]」の責務を持つ。MIDI 入力の場合は `text/` を経由せず `prosody/midi-to-frames.ts` を直接使い、Auto Sing は `prosody/melody-to-frames.ts` を使う、という対称的な構造になる。Phase 8 の現設計が `phoneme-timeline.ts` を `src/text/` 配下に置くと、後から MIDI 経路や Auto Sing 統合を試みた時に「テキスト由来でない韻律処理を `src/text/` に置くのか？」というディレクトリ違和が生じる。境界線の引き方の設計責任は Phase 8 ではなく Phase 1 に遡る問題で、Phase 1 段階で「韻律とテキストは別レイヤ」と決めておけばこの違和は発生しない。
...
より厳密には、Phase 8 の `phoneme-player.ts` は (a) `AudioContext.currentTime` を読む (b) `setTimeout` でメインスレッド側のイベント発火を仕掛ける (c) `engine.scheduleTransition` 経由で Worklet に postMessage する、という 3 種の副作用を持っており、これらは全て「再生時刻」の異なる表現に依存している。`AbstractClock` を導入してもまだ不十分で、本来は `Frame[]` を生成する純粋関数層と、`Frame[]` を消化する再生層を完全に分離し、再生層を `RealtimePlayer` / `OfflinePlayer` の 2 実装にする必要がある。`OfflinePlayer.render(frames): Promise<AudioBuffer>` は `OfflineAudioContext` 内で全 Frame を `scheduleTransition` 相当の処理に流し込むだけになり、wav エクスポート機能は 50 行程度で実装できる。Phase 8 の `phoneme-player.ts` を最初から「Frame 配列 → 副作用」の関数型 API（`renderFrames(frames, clock, engine)`）として書いておけば、リアルタイム / オフラインの差異は引数の `clock` だけになる。現状の Phase 8 設計は `PhonemePlayer` クラス内部に `AudioContext` 参照を持たせる前提で書かれているため、Phase 10 でオフラインレンダリング拡張する際にクラス分割からやり直しになる懸念がある。**Phase 8 で先回りして対処するなら**、`PhonemePlayer` のコンストラクタで `clock: { now(): number; setTimer(delay, fn): CancelToken }` インタフェースを受け取る形にしておくだけでよく、Phase 8 のコード量は 10 行程度しか増えない。
...
ただし**YAGNI 原則とのバランス**を冷静に評価する必要がある。ここまで挙げた `Frame` / `SequencerBase` / `Prosody` / `AbstractClock` の 4 抽象を Phase 1 で全て導入していたら、Phase 1 のスコープは「単一母音の連続音発生」だけだったにも関わらず、その時点では使われない抽象基底クラスが 4 つも存在することになる。Phase 1 の段階では「自分が将来 Auto Sing と TTS とオフラインレンダリングを実装する」という確信は無く、実際には Phase 4 まで進んで初めて「タイミング管理が散乱する」痛みが顕在化する。**この種の設計は Phase 4 か Phase 6 のどこかで「リファクタリング専用フェーズ」を 1 つ挟む方が現実的**で、Phase 1 で過剰設計するのは YAGNI 違反になる。実際の最善手は (a) Phase 1 では現状通りシンプルに作る (b) Phase 3-4 完了時点で `auto-singer` と `vowel-preset` の重複を見て `SequencerBase` を抽出する小規模リファクタを 1 フェーズ挟む (c) Phase 8 着手前にもう一度「Phase 8 と既存層の重複度」を測ってから抽象化レベルを決める、という段階的アプローチである。Phase 8 で「一から作り直すなら」の観点が価値を持つのは、**Phase 9 以降の MIDI / wav エクスポート / 多言語拡張**で同じ過ちを繰り返さないためであり、過去の Phase 1-7 への遡及修正を意味するものではない。本セクションの記述は将来フェーズへの設計指針として読むべきで、既に動いている Phase 1-7 の書き直し提案ではない、と明確に位置付けておく。
