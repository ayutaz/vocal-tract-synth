...
**8. モジュール配置 (`models/` vs `audio/`)**: 現行は `audio/vocal-tract.ts`, `audio/glottal-source.ts` のように音響処理を全て `audio/` 下に置いている。Phase 7 で `NasalTract`, `VelopharyngealPort`, （将来の）`Articulator`, `JunctionGraph` を追加すると `audio/` が肥大化し、「Worklet 関連のグルー (worklet-processor.ts, message-protocol.ts)」と「物理モデル本体」が混在する。再設計版なら `models/acoustic/` ディレクトリを切り、`AcousticTube`, `VocalTract`, `NasalTract`, `VelopharyngealPort`, `JunctionGraph` などの純粋物理モデルを集約する。`audio/` には Worklet 統合層（プロトコル、Worklet エントリ、AudioParam 配線）のみを残す。これにより物理モデルは Worklet 非依存で単体テスト可能になり、Phase 11 以降の WASM 化や Node.js 側でのオフライン合成にも流用できる。Phase 7 段階で大規模リファクタを行うコストは高いが、`audio/vocal-tract.ts` を `models/acoustic/vocal-tract.ts` に移動して再エクスポートする最小限の整理だけは検討余地がある。
 
**9. 鼻腔管断面積の静的 vs 動的**: 現行は `NASAL_AREAS` を Dang らの計測値から導出した固定 44 要素配列として `nasal-tract.ts` 内に const 定義し、ランタイム変更を想定しない。これは「日本語の鼻音 3 種は全て鼻腔形状が同じ」という単純化に基づく合理的な選択。しかし将来的に (a) 鼻詰まり表現（鼻孔側数区間の断面積を縮小） (b) 個人差（成人男性 vs 女性 vs 小児で鼻腔長と容積が異なる） (c) 病理音声シミュレーション（鼻茸・粘膜浮腫）を扱う可能性がある。再設計版なら Phase 1 から `NasalTract` を `setAreas(areas: Float64Array)` API で外部から書き換え可能にしておき、デフォルト値として `DEFAULT_NASAL_AREAS` 定数を提供する。`AcousticTube` 基底クラスに `setAreas()` を持たせれば追加コストはゼロ。リアルタイム変更時の反射係数更新は dirty フラグ方式（Phase 6 設計と同じ）で十分。Phase 7 段階では UI からの動的変更は実装しないが、API だけ準備しておけば Phase 10 以降の「個人差プリセット」「健康・病理切替」機能を低コストで追加できる。

**10. velum 制御の単位と物理的妥当性**: 現行は `velopharyngealArea: 0-2.0 cm²` という連続値で軟口蓋開度を表現するが、これは「3 ポート接合の側枝側面積」という数学的パラメータであり、解剖学的な「軟口蓋の咽頭壁からの距離 (mm)」「鼻咽腔の通過断面積」と直接対応するわけではない。実際の軟口蓋運動は (a) 上後方への持ち上げ (levator veli palatini) (b) 鼻咽腔閉鎖の有無は二値に近い（完全閉鎖 or 開放、その中間状態は短時間） (c) 開放時の有効断面積は 0.5-1.5 cm² 程度、という生理学的特性を持つ。再設計版なら `VelopharyngealPort` クラスに `setOpening(openingMm: number)` のような解剖学的単位の API と、内部で `mm → cm²` の変換テーブル（生理計測に基づく非線形マップ）を持たせる方式が望ましい。瞬時切替 vs 時定数付き遷移については、生理学的には軟口蓋運動の時定数は 30-80 ms 程度であり、現行の 5-10 ms ランプより遅い。Phase 7 では音響的なクリック回避を優先して短いランプを使うが、再設計版なら「音響クリック回避用の高速ランプ (5 ms)」と「生理学的に正しい遷移 (50 ms)」を切替可能にしておく価値がある。

**11. 計算量と分岐戦略**: 現行設計では velum=0（純母音）でも NasalTract の `processSample` を呼び出し、44 区間の散乱ループを実行する可能性がある（Phase 7 設計では `if (velum < 1e-6) skip` の早期 return を入れる方針だが、その判定自体が分岐予測のコスト）。母音中心の利用パターン（実際の発話で鼻音は 5-15% 程度）を考えると、「鼻腔管を常時計算する設計」は無駄が大きい。再設計版なら 3 つの選択肢がある: (a) 現行の「条件付きスキップ」方式: シンプルだが分岐予測ミスのリスク (b) 「鼻音モードフラグで関数ポインタ切替」方式: `processSample` 自体を `processSampleOral()` と `processSampleNasalCoupled()` の 2 種類用意し、`setNasalCoupling` 時に切替: 分岐削減で SIMD 化と相性が良い (c) 「常時計算 + 出力ミックス係数」方式: velum=0 でも鼻腔散乱を回し、出力時に `oralOut * (1-mix) + nasalOut * mix` で混ぜる: 分岐ゼロで予測ミスもないが計算量は常に 2 倍。`process()` 内のフレーム単位ベンチマークを計測した上で (b) を採用するのが現実的。ただし JIT の関数インライン化を妨げないよう、関数ポインタではなく `if (this.nasalCoupled) processNasal(); else processOral();` の ifelse 分離（オブジェクト全体の hot/cold path 分岐）の方が V8 最適化と相性が良い場合もある。Phase 7 段階で計測ベースの選択を行うべき。

**実装順序（再設計版）**: ①types/parameters → ②AcousticTube 基底（`models/acoustic/`） → ③Worklet 通信プロトコル (scheduleTransition + setNasalCoupling 含む) → ④turbulence-noise + VelopharyngealPort モジュール → ⑤VocalTract + NasalTract（同時実装、共通基底ベース） → ⑥Articulator (3 ポート接合含む、hot/cold path 分岐対応) → ⑦声門音源 → ⑧プリセット (母音 + 子音 + 鼻音) → ⑨UI → ⑩Auto Sing → ⑪テキスト読み上げ。これで Phase 6 + 7 の後付け改修コストの大半が消える。ただし Phase 1 段階での設計検証時間が 2-3 日増えるため、「Phase 1-5 を母音楽器 MVP と割り切る」なら現行の単純設計の方が合理的で、Phase 6-9 の改修コスト（各 5 日程度）を「拡張時に支払う技術的負債」として受け入れるトレードオフも妥当な選択。
