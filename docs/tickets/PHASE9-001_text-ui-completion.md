...
一から作り直すなら、まず Phase 1 の段階で `OperationMode` を**有限状態機械（FSM）**として設計し、`Mode` という抽象クラスを定義する。各モードは `Mode.enter()` / `Mode.exit()` / `Mode.handleEvent(event)` の 3 メソッドを持ち、UI 要素の enable/disable は `Mode.enter()` 内で完結させる。Phase 1 の段階では `ManualMode` クラスのみ存在し、Phase 4 で `AutoSingMode` を追加、Phase 9 で `TextReadMode` を追加、という設計にする。`Mode` クラスは `ModeManager` から呼び出され、`ModeManager` が遷移可能性とコールバック発火を担当する。これにより各モードの責務が明確化され、新モード追加が「新クラスを 1 つ書くだけ」で済む構造になる。重要なのは `enter()` / `exit()` を**副作用管理の境界**として位置付ける点で、`AutoSingMode.enter()` で `TransitionManager` を生成し `exit()` で破棄、`TextReadMode.enter()` で `phonemePlayer.start()` を呼び `exit()` で `stop()` する、というリソースのライフサイクルがモードのライフサイクルと一致する。現状の Phase 9 設計では `applyModeToUI` が UI 要素の enable/disable のみ担当し、リソース取得/解放は `phonemePlayer` 側の `play()` / `stop()` 呼び出しで分散管理されているため、「モード遷移の途中で例外が出た時どこまでロールバックすべきか」が曖昧である。`enter()` / `exit()` を `try/finally` で囲む規約にすれば、`TextReadMode.exit()` が必ず `phonemePlayer.stop()` を呼ぶことが保証され、状態リークが構造的に防げる。
...
Flux/Redux 的な単一ストア設計についても言及しておく。現在のプロジェクトは状態管理ライブラリを使わず、`tractEditor` / `engine` / `autoSinger` / `phonemePlayer` などが各自の内部状態を持ち、メッセージパッシングで連携する分散型である。Phase 9 で 3 モード × 6 クラスの状態が絡むと「どこに真の状態があるのか」が分かりづらくなる。一から作り直すなら、Zustand のような軽量ストアライブラリ（バンドルサイズ 1 KB 以下）を導入し、`{ mode, isPlaying, currentPhonemeIndex, f0, volume, ... }` のような単一ストアで管理する設計を検討する。但し YAGNI 観点では Phase 9 までの規模では分散型で十分動作し、Phase 1 で導入してもオーバーエンジニアリングになる可能性が高い。Phase 1 では「単一ストアにすべき/分散型でよい」の判断保留が正解で、Phase 9 で 3 モード × 6 クラスの規模に達した時点で初めて「リファクタリング機会」として再評価する、という段階的判断が現実的である。

3 モードの抽象化レベルについて。`manual` モードは「停止中の `textRead` モード」「停止中の `autoSing` モード」とも捉えられる。つまり厳密には 3 モードではなく「`autoSing.state ∈ { stopped, playing }` × `textRead.state ∈ { stopped, playing }` × 排他制約」という 2 軸の状態である。但しこの設計は状態空間が複雑化（4 状態のうち 1 つは禁止）し UI 側のロジックが分かりづらくなるため、Phase 9 では「3 値の単純な enum」を採用する。

モード設計でより本質的に難しいのは「**どの状態がモード横断か、どの状態がモード依存か**」の分類である。一から作り直すなら Phase 1 の段階でこの分類を明示的に決めておく。声質パラメータ（Rd, OQ, Aspiration）と基準 F0、声道の 16 制御点、音量は**モード横断**として全モードで共有する（手動で形を作って Auto Sing で歌わせる、という用途が前提のため）。一方 Auto Sing の Note 列、テキスト読み上げの音素列、現在再生位置、TransitionManager の補間中状態は**モード依存**で `Mode` クラスのインスタンス変数として保持する。`ManualMode` から `AutoSingMode` への遷移時、声道形状はそのまま引き継がれ、Note 列は `AutoSingMode.enter(state)` 内で初期化される。この境界が現状 Phase 9 では曖昧で、特に Phase 4 の Auto Sing 追加時に「ピッチスライダー（モード横断、基準 F0）と Note 列の F0（モード依存）を合算する」という決定が ad hoc に行われ、Phase 9 のテキスト読み上げでも「ピッチスライダーで全体を移調」を実装するか否かで設計判断が必要になる。Phase 1 で「モード依存 / モード横断」表を作っておけば、新モード追加時の判断基準として機能する。同様に Phase 4 の「Auto Sing 中は声道ドラッグ無効化」も、本質的には「声道形状はモード横断だが書き込み権限は Mode が独占する」という ownership ルールであり、Phase 1 で `state.tractShape.owner: 'manual' | 'autoSing' | 'textRead'` のような所有権モデルを明示すれば曖昧さが消える。

レイアウトシステムについても、現在の `display: flex` 素朴配置を CSS Grid + `grid-template-areas` の 4 領域定義（`tract-editor` / `spectrum` / `mode-controls` / `timeline`）に置き換える案があり、レスポンシブ対応や将来的なパネル位置変更が容易になる。Phase 1 から `grid-template-areas` で領域を命名しておけば、Phase 9 で `#text-read-controls` と `#phoneme-timeline-canvas` を追加する際に「どの領域に入れるか」が CSS 側で完結し、HTML 構造を変更せずレイアウトを差し替えられる。さらにモバイル/タブレット対応として `@media (max-width: 768px)` で領域を再配置することも Phase 1 から考慮できる。但し Phase 9 では既存 flex レイアウトを尊重し `#text-read-controls` を `#auto-sing-controls` 直下に追加するだけで進める。Web Component 化（`<text-read-controls>` 等のカスタム要素）は理論的選択肢として、HTML 側で `<text-read-controls>` と書くだけで UI が自動配置される宣言的記述の利点があり、特に Phase 9 のように「複数モードの UI パネルを並べる」用途では Web Component の自己完結性が活きる。但し Shadow DOM の CSS 共有困難性が `style.css` 単一管理と相性が悪く、`:host` セレクタや CSS Custom Properties を介した間接スタイリングが必要になり初学者の保守性が下がるため、本フェーズでは採用しない。

最後に、Phase 9 を一から作り直す観点で最も価値があるのは「**Phase 1 から OperationMode を State Machine として設計し、状態の所有権モデル（モード横断 / モード依存 / モード独占書き込み）を明示する**」という 2 点に尽きる。前者は「いずれ複数モードになる」ことが Phase 1 の段階でも予測可能（Auto Sing は要件定義時点で計画されていた）であり、後者は要件定義時点で「手動で形を作って Auto Sing で歌わせる」というユースケースが明記されているため、状態共有が必須であることが先読みできた。`CanvasLayer` 基底クラス、`AppController` 統合 facade、Web Component 化、Redux 的ストア、grid-template-areas レイアウトはいずれも YAGNI 違反になりがちで、Phase 1 時点では「最初の Canvas 1 枚に対して基底クラスを抽出する」ような早期抽象化は逆効果になる。これらは Phase 5 程度の規模（Canvas 2 枚目、UI クラス 3 つ目）で「3 つ目を追加するタイミング」に初めてリファクタリングするのが現実的である。Observer パターン化も Phase 4 の Auto Sing 追加時点で「2 モードあるなら subscribe させた方が綺麗」と判断できるため Phase 4 で導入が妥当であり、Phase 1 から準備する必要はない。本セクションの教訓は、過去フェーズへの遡及修正提案ではなく、**次の声道シンセサイザー風プロジェクトの Phase 1 設計指針**として、また Phase 10 以降のリファクタリングフェーズの優先順位指針として記録すべきものである。
