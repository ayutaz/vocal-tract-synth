...
代替案として「制御点を 32 点に増やす」案も検討に値するが、(a) UI ドラッグ操作の認知負荷が増す (b) 32 点でも /s/ の鋭い狭窄 (4 区間幅) を捉えるには境界がきわどい (c) 16 点と 44 区間の比 (2.75) と異なり 32 点では 1.375 となり整数比でない補間誤差が増える、という三つの理由で却下する。代わりに「**狭窄点オーバーライド機構**」(`overrideSection(index, area)` のように 44 区間の任意 1〜数区間を直接書き換え、スプライン補間結果に上書きを掛ける) を Phase 1 から入れておくのが最良。これにより UI のメンタルモデル (16 制御点) を変えずに、子音プリセットが必要とする局所狭窄を物理的に正確に表現できる。Phase 6 では `consonant-presets.ts` 内で「先行母音の 16 点 → スプライン → 44 区間」を生成した後に子音狭窄区間のみ上書きする計算を呼び出し側で行っているが、再設計版ではこれが `VocalTract` の一級 API になる。
...
具体的には、Phase 1 から Worklet 内に「タイムラインエンジン」を載せておくべきだった。タイムラインエンジンの責務は (a) サンプルカウンタの保持 (b) 「t=N サンプル時に areas/noise/glottal を X に変更」というイベントキューの保持と消化 (c) 各イベント間のサンプル精度補間 の 3 つに絞り、物理計算 (Kelly-Lochbaum, 声門音源) とは疎結合にする。Phase 6 で導入する `transitionActive` / `transitionStartAreas` / `transitionInterimAreas` などはタイムラインエンジンの一機能として吸収される。Phase 1 段階ではエンジンに 1 種類のイベント (areas 変更) しか乗らないため過剰設計に見えるが、Phase 4 のジッター/シマー (k-rate より細かい変動) や Phase 5 の Auto Sing (ピッチ/フォルマント遷移) もこのエンジンに乗せられるため、後付けコストが消える。
...
**MIN_AREA の設計**: 現行は Phase 1 で `MIN_AREA = 0.3` をハードコーディングし、Phase 6 で `MIN_AREA_PROGRAM = 0.01` を追加する二段化にしている。再設計するなら最初から **`MIN_AREA = 0.01` 単一定数で統一**し、UI ドラッグ側のクランプはレイヤを分けて `UI_MIN_DRAG_AREA = 0.3`（あるいは「ユーザの指がそれより細い値を狙えない」物理的制約として `pointer-coarse` メディアクエリで動的調整）として定義するのが筋が良い。理由は (a) 「物理モデルとして 0.01 が安全に扱えるか」は UI とは独立に Phase 1 で検証すべき性質である (b) 二段化は名前空間に `_PROGRAM` 接尾辞という不自然な区別を持ち込み、コードリーダーに「なぜ二つあるのか」を毎回考えさせる (c) 子音や鼻音の閉鎖が必要になった瞬間に二段化が必要になることは、Phase 1 段階の数値安定性検証 (反射係数が 1.0 に漸近する境界の探索) で十分予見できる、の三点。

特に重要なのは「**反射係数の数値安定性は Phase 1 で検証すべきだった**」という点。Phase 1 では母音の安定性のみを検証し、`MIN_AREA = 0.3` での `r = (4.0 - 0.3) / (4.0 + 0.3) ≈ 0.86` という安全な範囲しか触れていない。もし Phase 1 で「面積 0.001〜10 の範囲で反射係数の絶対値が 1.0 を超えないか」「壁面損失 mu = 0.999 と組み合わせた時の累積エネルギーが発散しないか」「Smith 1 乗算接合と完全閉鎖の数値特性 (NaN/Inf 発生条件)」を体系的にテストしておけば、Phase 6 で 6.2 のチェック項目を追加する必要がなかった。再設計版では Phase 1 の `vocal-tract.test.ts` に「断面積を 0.001 → 100 まで 30 段階で振る境界値テスト」と「全区間閉鎖を 10 秒シミュレートして発散しないテスト」を最初から含める。

二段制限が「過度な複雑性」かどうかは判断が分かれる。現行のように UI 層と物理層の区別が明確な (UI レイヤは `tract-editor.ts` に集約されている) アーキテクチャでは、定数を二つに分けるよりも「UI レイヤだけがクランプを掛け、物理層は受信値をそのまま信頼する」契約のほうが自然。再設計版ではこの契約を採用し、`MIN_AREA` という単一概念を物理層に置く。
...
**ノイズ注入の配置**: VocalTract クラス内に置く現行案は GC-free のために合理的だが、責務としては「乱流発生 + フィルタ整形」という独立した DSP モジュール (`turbulence-noise.ts`) に切り出し、VocalTract が依存性注入で受け取る形が望ましい。再設計版なら最初からこの切り出しを行い、テストも独立して書ける (UT-03, UT-04 が VocalTract 全体に依存しない)。具体的には `NoiseSource` インターフェース (`generateSample(): number` のみ) を定義し、LCG + Biquad BPF を実装した `TurbulenceNoiseSource` クラスを VocalTract のコンストラクタに注入する。Phase 1 段階では VocalTract がこのソースを「ノイズなし」(常に 0 を返すヌル実装) で受け取り、Phase 6 で実装を差し替えるだけで済む。

**`VocalTractFilter` インターフェースの抽出**: 現行は `GlottalModel` インターフェース (KLGLOTT88 と LF を切り替え可能にする抽象) は存在するが、`VocalTract` は具象クラスのまま使われている。子音対応で `setConstrictionNoise` / `scheduleTransition` / `overrideSection` といった API が増えるにつれ、VocalTract の表面積は膨張する一方となる。再設計版では `VocalTractFilter` インターフェースを Phase 1 で定義し、

```typescript
interface VocalTractFilter {
  processSample(input: number): number;
  setAreas(areas: Float64Array): void;
  injectNoise(index: number, amount: number): void;  // Phase 1 から
  overrideSection(index: number, area: number): void;
  getCurrentAreas(): Readonly<Float64Array>;
}
```

のように「散乱モデルの具体実装に依存しない」最小 API を定めておく。Kelly-Lochbaum 1 乗算接合は最初の実装にすぎず、将来的に WDF (Wave Digital Filter) や TLM (Transmission Line Matrix) などの別実装に差し替える余地を残せる。`injectNoise(index, amount)` を Phase 1 から組み込む判断は、(a) 母音発声では `amount = 0` 固定で動作するため副作用がない (b) インターフェースに含めておけばテストで「ノイズ注入時に該当区間の前進波が確かに加算されている」ことを単体検証できる (c) Phase 6 で API 追加 = 既存テスト全件の再検証が必要、というコストを Phase 1 で前払いできる、の三点で妥当。逆に Biquad BPF や LCG はノイズソースの内部実装詳細であるため `VocalTractFilter` には露出させず、`NoiseSource` 側に閉じ込める。
